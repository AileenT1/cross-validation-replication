...
here::i_am("code/03_sliding_linear.R")
...
rds_path <- file.path(results_dir, "03_sliding_linear_results.rds")

sliding_linear_results <- if (file.exists(rds_path)) readRDS(rds_path) else NULL
if (!is.null(sliding_linear_results) &&
    all(c("rv_config", "error_rate_by_n") %in% names(sliding_linear_results))) {
  linear_truth_rv_config <- sliding_linear_results$rv_config
...
#Graphing and tables
sliding_linear_settings_tbl <- data.frame(
  setting = c(
