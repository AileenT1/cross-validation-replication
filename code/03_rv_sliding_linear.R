...
here::i_am("code/03_rv_sliding_linear.R")
...
rds_path <- file.path(results_dir, "03_rv_sliding_linear_results.rds")

sliding_linear_results <- if (file.exists(rds_path)) readRDS(rds_path) else NULL
if (is.null(sliding_linear_results) ||
    !all(c("simulation_config", "model_fitting_config", "error_rate_by_n") %in% names(sliding_linear_results))) {
  set.seed(2026)
  ...
  simulation_config <- sliding_linear_results$simulation_config
  model_fitting_config <- sliding_linear_results$model_fitting_config
  error_rate_by_n <- sliding_linear_results$error_rate_by_n
  message("Loaded results from ", rds_path)
}

sliding_linear_simulation_settings_tbl <- data.frame(
  setting = c(
    "seed",
...
#tables&graphs
sliding_linear_error_rate_summary_tbl <- error_rate_by_n |> arrange(n)
sliding_linear_error_rate_plot <- ggplot(error_rate_by_n, aes(x = n, y = error_rate)) +
  geom_line(linewidth = 0.35) +
  labs(
    x = "Sample size",
    y = "RV selection error rate",
    title = "RV selection error rate (sliding window, linear truth)"
  ) +
  scale_x_continuous(
    limits = c(0, 1000),
