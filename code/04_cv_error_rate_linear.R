...
here::i_am("code/04_cv_error_rate_linear.R")
...
rds_path <- file.path(results_dir, "04_cv_error_rate_linear_results.rds")
cv_linear_results <- if (file.exists(rds_path)) readRDS(rds_path) else NULL

if (is.null(cv_linear_results) ||
    !all(c("simulation_config", "model_fitting_config", "error_rate_by_n") %in% names(cv_linear_results))) {
  set.seed(2026)
  ...
  simulation_config <- cv_linear_results$simulation_config
  model_fitting_config <- cv_linear_results$model_fitting_config
  error_rate_by_n <- cv_linear_results$error_rate_by_n
  message("Loaded results from ", rds_path)
}

# tables & graphs
cv_linear_simulation_settings_tbl <- data.frame(
  setting = c(
    "seed",
...
cv_linear_error_rate_summary_tbl <- error_rate_by_n |> arrange(n)
max_n <- max(simulation_config$sample_sizes)
cv_linear_error_rate_plot <- ggplot(error_rate_by_n, aes(x = n, y = error_rate)) +
  geom_line(linewidth = 0.35) +
  labs(
    x = "Sample size",
    y = "CV selection error rate",
    title = "CV selection error rate (linear truth)"
  ) +
  scale_x_continuous(
    limits = c(0, max_n),
