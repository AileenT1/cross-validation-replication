...
here::i_am("code/03_expanding_poly.R")
...
rds_path <- file.path(results_dir, "03_expanding_poly_results.rds")

expanding_poly_results <- if (file.exists(rds_path)) readRDS(rds_path) else NULL
if (!is.null(expanding_poly_results) &&
    all(c("rv_config", "error_rate_by_n") %in% names(expanding_poly_results))) {
  poly_truth_rv_config <- expanding_poly_results$rv_config
...
#Graphing and tables
expanding_poly_settings_tbl <- data.frame(
  setting = c(
