here::i_am("code/02_cv_error_rate.R")

report_only <- isTRUE(getOption("cv_error_rate_02_report_only", FALSE))
...
if (report_only) {
  ...
  rds_path <- here::here("data", "final", "02_cv_error_rate_results.rds")
  if (!file.exists(rds_path)) {
    stop(
      "Missing results file: data/final/02_cv_error_rate_results.rds. ",
      "Run code/02_cv_error_rate.R first (e.g. Rscript code/02_cv_error_rate.R)."
    )
  }
  ...
  error_rate_plot <- ggplot2::ggplot(by_n, ggplot2::aes(x = n, y = error_rate)) +
    ...
    ggplot2::scale_y_continuous(limits = c(0, 0.3), expand = ggplot2::expansion(mult = c(0, 0.02))) +
    ggplot2::theme_bw()
} else {
  ...
  K <- 10L
  mc_runs <- 1000L
  sample_sizes <- seq(50L, 10000L, by = 50L)
  beta0 <- 1
  beta1 <- 2
  sigma <- 4
  poly_degree <- 3L
  ...
  output <- list(
    parameters = list(
      ...
      K = K,
      mc_runs = mc_runs,
      sample_sizes_label = "50, 100, ..., 10000 (step 50)",
      beta0 = beta0,
      beta1 = beta1,
      sigma = sigma,
  ...
  saveRDS(output, file = file.path(results_dir, "02_cv_error_rate_results.rds"))

  message("Saved results to ", file.path(results_dir, "02_cv_error_rate_results.rds"))
}
