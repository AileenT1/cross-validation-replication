here::i_am("code/01_run_cv_linear_truth.R")

report_only <- isTRUE(getOption("_01_cv_linear_truth_report_only", FALSE))

if (report_only) {
  suppressPackageStartupMessages(library(dplyr))
  rds_path <- here::here("data", "final", "01_cv_linear_truth_results.rds")
  if (!file.exists(rds_path)) {
    stop(
      "Missing results file: data/final/01_cv_linear_truth_results.rds. ",
      "Run code/01_run_cv_linear_truth.R first (e.g. Rscript code/01_run_cv_linear_truth.R)."
    )
  }
  ...
  selection_summary_tbl <- selection_summary |>
    arrange(n, selected_model)
  gap_summary_tbl <- gap_summary |>
    arrange(n)
} else {
  ...
  message("Saved results to ", file.path(results_dir, "01_cv_linear_truth_results.rds"))
}
